...
class ExceptionHandler{
   ...
   */
  public static function setTemplate(string $file){
    if(!file_exists($file)){
      throw new \Exception(str_replace('{%file}', self::$lang, self::getLang()::FILE_NOT_EXISTS));
    }
    self::$template = $file;
  }
  ...
  public static function setLang(string $lang){
    ...
    if(!array_key_exists($lang, self::$langsAvailable)){
      throw new \Exception(str_replace('{%lang}', $lang, self::getLang()::NO_LANG_DATA));
    }
    self::$lang = $lang;
  }
   ...
   */
  public static function setWriter(\Logguer\Writer $writer){
    self::$writer = $writer;
  }
  ...
  public function __construct(\Throwable $exception, $caught = false){
  ...
  public function displayHTML(\Throwable $exception, $caught){
    $textCaughtException = $caught ? self::getLang()::CAUGHT_EXCEPTION : self::getLang()::UNCAUGHT_EXCEPTION;
    $textFileAndLine = str_replace('{%file}',
                                    $exception->getFile(),
                                    str_replace('{%line}',
                                                $exception->getLine(),
                                                self::getLang()::FILE_AND_LINE
                                    )
                        );
